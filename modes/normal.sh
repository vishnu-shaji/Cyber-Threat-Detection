...
if [[ "$REPORT" = "1" ]]; then
  ...
  args="$args --noreport"
  threat $args | tee $LOOT_DIR/output/threat-$TARGET-`date +"%Y%m%d%H%M"`.txt 2>&1
  exit
fi
...
echo -e "${OKGREEN}====================================================================================${RESET}•x${OKGREEN}[`date +"%Y-%m-%d](%H:%M)"`${RESET}x•"
for a in `cat $LOOT_DIR/nmap/nmap-$TARGET.xml | grep state\=\"open\" | grep http | grep -v https | grep -v ssl | grep tcp | cut -d\" -f4`; do threat -t $TARGET -m webporthttp -p $a -w $WORKSPACE; done;

echo -e "${OKGREEN}====================================================================================${RESET}•x${OKGREEN}[`date +"%Y-%m-%d](%H:%M)"`${RESET}x•"
echo -e "$OKRED SCANNING ALL HTTPS PORTS $RESET"
echo -e "${OKGREEN}====================================================================================${RESET}•x${OKGREEN}[`date +"%Y-%m-%d](%H:%M)"`${RESET}x•"
for a in `cat $LOOT_DIR/nmap/nmap-$TARGET.xml | grep state\=\"open\" | egrep 'https|ssl' | grep tcp | cut -d\" -f4`; do threat -t $TARGET -m webporthttps -p $a -w $WORKSPACE; done;
