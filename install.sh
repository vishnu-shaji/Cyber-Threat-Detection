...
LOOT_DIR=/usr/share/threat/loot
PLUGINS_DIR=/usr/share/threat/plugins
GO_DIR=~/go/bin

echo -e "$OKRED[>]$RESET This script will install threat under $INSTALL_DIR. Are you sure you want to continue? (Hit Ctrl+C to exit)$RESET"
if [[ "$1" != "force" ]]; then
	read answer
fi
...
chown root $INSTALL_DIR/threat 2> /dev/null
...
rm -f /usr/share/threat/templates/active/CVE*
...
wget https://raw.githubusercontent.com/janmasarik/resolvers/master/resolvers.txt -O /usr/share/threat/wordlists/resolvers.txt
...
chmod +x $INSTALL_DIR/threat
chmod +x $PLUGINS_DIR/Goohak/goohak
rm -f /usr/bin/dirsearch
ln -s $INSTALL_DIR/threat /usr/bin/threat 2> /dev/null
...
ln -s /usr/share/threat /threat 2> /dev/null
ln -s /usr/share/threat /usr/share/threat 2> /dev/null
ln -s /usr/share/threat/loot/workspace /workspace 2> /dev/null
ln -s /usr/share/threat/loot/workspace /root/workspace 2> /dev/null
ln -s /usr/share/threat /root/threat 2> /dev/null
ln -s /root/.threat.conf /usr/share/threat/conf/threat.conf 2> /dev/null
ln -s /root/.threat_api_keys.conf /usr/share/threat/conf/threat_api_keys.conf 2> /dev/null
mv /root/.threat.conf /root/.threat.conf.bak 2> /dev/null
cp -vf /usr/share/threat/threat.conf /root/.threat.conf 2> /dev/null
...
mkdir -p /usr/share/threat/loot/workspaces/ 2> /dev/null
ln -fs /usr/share/threat/loot/workspaces/ /home/kali/Desktop/workspaces 2> /dev/null
ln -fs /usr/share/threat/loot/workspaces/ /root/Desktop/workspaces 2> /dev/null
